...
public class OBJModel {
	...
	private final ArrayList<OBJIndex> m_indices;
		...
		try {
			...
			while ((line = meshReader.readLine()) != null) {
				...
				if (tokens.length == 0 || tokens[0].equals("#")) {
					...
					m_positions.add(new Vector3f(Float.valueOf(tokens[1]), Float.valueOf(tokens[2]), Float.valueOf(tokens[3])));
				} else if (tokens[0].equals("vt")) {
					m_texCoords.add(new Vector2f(Float.valueOf(tokens[1]), 1.0f - Float.valueOf(tokens[2])));
				} else if (tokens[0].equals("vn")) {
					m_normals.add(new Vector3f(Float.valueOf(tokens[1]), Float.valueOf(tokens[2]), Float.valueOf(tokens[3])));
	...
	public IndexedModel ToIndexedModel() {
		...
		for (int i = 0; i < m_indices.size(); i++) {
			final OBJIndex currentIndex = m_indices.get(i);

			final Vector3f currentPosition = m_positions.get(currentIndex.GetVertexIndex());
